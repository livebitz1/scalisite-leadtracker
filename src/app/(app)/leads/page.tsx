...
export default async function LeadsPage({
  ...
  return (
    <div className="animate-fade-in">
      ...
      {!isAdmin && (
        <div className="mb-5 inline-flex rounded-lg border border-line bg-white/[0.03] p-1 text-sm">
          <Link
            href="/leads"
            className={`rounded-md px-3 py-1.5 transition ${
              !mine ? "bg-white text-black" : "text-white/55 hover:text-white"
            }`}
          >
            All leads
          </Link>
        ...
        <div className="space-y-3 md:hidden">
          {leads.map((lead) => (
            <Link
              key={lead.id}
              href={`/leads/${lead.id}`}
              className="card block p-4 transition active:bg-white/[0.04]"
            >
              ...
              <div className="mt-3 flex items-center justify-between gap-3 border-t border-line/60 pt-3 text-xs">
                <span className="inline-flex items-center gap-1.5 text-white/55">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full border border-line bg-white/5 text-[9px] font-semibold text-white/70">
              ...
              <tbody>
                {leads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-white/60">{lead.source}</td>
                    <td className="px-5 py-3.5">
                      <span className="inline-flex items-center gap-2">
                        <span className="flex h-6 w-6 items-center justify-center rounded-full border border-line bg-white/5 text-[10px] font-semibold text-white/70">
                          {initials(lead.assignedTo.name)}
                        </span>
                    ...
                    <td className="px-5 py-3.5 text-right tabular-nums text-white/80">
                      {formatCurrency(lead.value)}
                    </td>
